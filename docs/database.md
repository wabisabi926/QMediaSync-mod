...
| `upload.status` | `0` 等待中、`1` 上传中、`2` 已完成、`3` 失败、`4` 已取消、`5` 远端已完成待本地收尾 |
...
- `file_name`：上传文件名。
- `status`：上传状态。115 上传在远端已确认完成、但本地结果落库和 STRM 入队等收尾尚未完成时，会短暂处于 `5`，队列展示为 `remote_completed_pending_finalize`。
- `file_size`：文件大小。
...
- 同一同步目录下允许多条规则，但完全相同的 `monitor_path + remote_root_path + remote_root_id` 组合会被拒绝。同步目录与规则通过聚合写接口在同一事务保存；更新同步目录时 `source_type` 和 `account_id` 必须与旧记录一致，不允许改写来源或账号。删除规则、关闭删除开关或修改监控边界时，会先取消相关 pending cleanup，再删除 processed 记录。

`sync_path_idempotency_records` 保存创建同步目录请求的 `Idempotency-Key` SHA-256 摘要、最终 `sync_path_id` 和状态，用于处理数据库已 commit 但客户端未收到响应后的重试。相同 key 会回放已创建聚合；如果该 key 仍处于处理中，则返回 `IDEMPOTENCY_CONFLICT`。原始 `Idempotency-Key` 不写入数据库。
...
- `request_hash` 只对 `pending` / `running` / `finalizing` / `waiting_children` 任务做幂等去重；如果历史任务已经 `failed`、`completed` 或 `cancelled`，再次提交同一请求会归档旧哈希并创建新任务。升级后新请求会写入短格式哈希；旧格式活跃任务仍可被相同请求复用。
