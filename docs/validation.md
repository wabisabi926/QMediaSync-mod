...
目录监控上传通过 `/api/directory-upload/sync-paths/:sync_path_id/rules` 统一保存配置，请求体为 `{"enabled": true, "rules": []}`。`enabled` 是同步目录总开关，写入 `sync_paths.directory_upload_enabled`；`rules` 是最终完整规则集合，已有规则未出现在请求中即视为删除，不再使用 `deleted_ids`。运行时只有总开关开启且规则自身 `enabled=true` 的规则生效；关闭总开关不会改写规则自身 `enabled`，方便下次开启时恢复上一次的规则启停状态。规则必须绑定明确的 `sync_path_id` 和账号；一个同步目录允许配置多条目录监控规则，但每条规则只能有一个 `monitor_path`。`monitor_path` 不得等于同步目录的 `LocalPath`，避免生成的 STRM 被再次上传；`remote_root_path` 必须位于对应 `SyncPath.RemotePath` 之下，才能安全映射 STRM 输出路径。同一同步目录下，完全相同的 `monitor_path + remote_root_path + remote_root_id` 组合会被拒绝；两个启用规则的监控目录不能重复。若一个启用规则递归监控父目录，另一个启用规则不能监控其子目录，避免同一源文件重复入队。统一保存接口会在事务中构造并校验最终规则集合，避免逐条更新时被旧规则状态误判；总开关开启时至少需要一条规则自身启用，总开关关闭时允许空规则集合。`watch_mode` 只允许 `auto`、`fsnotify`、`polling`，同名文件处理方式只允许 `skip_same`、`fail_conflict`、`replace_conflict`，不兼容旧 `watcher` / `always` 值。`recursive=false` 只允许根目录文件进入稳定性队列，启动扫描、补偿扫描和 fsnotify 新建子目录事件都不得绕过该限制。`ignore_patterns` 使用数组请求和响应，后端以字符串字段持久化但列表接口必须回传解析后的数组，避免更新其他字段时清空已有忽略规则。稳定窗口、稳定检查间隔和补偿扫描间隔为代码内置参数，接口不接收也不采纳 `stability_seconds`、`stability_check_interval_seconds`、`stability_required_count`、`rescan_interval_seconds`。`upload_metadata` 是显式开关，默认关闭；开启后候选文件扩展名会从“当前同步目录视频扩展名”扩展为“当前同步目录视频扩展名或元数据扩展名”，两者都沿用同步目录自定义配置优先、为空回退全局 STRM 设置的规则。上传后删除源文件是显式开关，默认关闭；开启后也只能在上传终态成功且 STRM 生成成功后执行，并且空目录清理不得删除监控根目录；清理规则优先由 `directory_upload_processed_files.upload_task_id` 反查入队时的 `rule_id`，找不到处理记录时才退回到历史路径匹配逻辑。`/api/directory-upload/sync-paths/:sync_path_id/scan` 只扫描总开关开启且规则自身启用的规则。

STRM Webhook 使用独立接口 `/api/strm/webhook`，鉴权沿用 API Key，支持 `X-API-Key` header 和 `?api_key=` 查询参数，当前仅支持 115 网盘来源。`sync_path_id` 可以显式提供；未提供时会按 115 远端路径自动匹配同步目录。文件级请求至少提供 `path + file_name`；显式提供 `sync_path_id` 时也可以使用 `file_id` 定位，`pick_code` 可以作为辅助字段传入但不能单独定位文件。目录级请求至少提供 `directory_path`；显式提供 `sync_path_id` 时也可以使用 `directory_id`。`download_meta` 和 `refresh_emby` 只能作为请求顶层字段传入，`items[]` 内出现这两个开关会拒绝整批请求。Webhook 请求中的 `path` / `directory_path` 只表示 115 远端路径，禁止接受或信任 `local_path` 来决定本地写入位置，本地 STRM 路径只能由显式指定或自动匹配到的同步目录计算。文件级请求会在入队前解析远端详情，并按解析后的真实路径再次校验同步目录边界。完整接口说明见 [STRM Webhook](strm-webhook.md)。
