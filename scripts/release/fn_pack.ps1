...
try {
    ...
    Write-Colored "green" "✓ 前端构建完成"
} catch {
    Write-Colored "red" "Error: Frontend build failed"
...
if (!(Test-Path -Path "backend/FNOS" -PathType Container)) {
    ...
    if (Test-Path -Path $arm64ManifestPath -PathType Leaf) {
        ...
        Write-Colored "green" "✓ Updated version in backend/FNOS/qmediasync-arm64/manifest to $TAG"
    } else {
    ...
    if (Test-Path -Path "temp_build/QMediaSync_linux_arm64_exe" -PathType Leaf) {
        ...
        Write-Colored "green" "✓ Copied arm64 executable to backend/FNOS/qmediasync-arm64/app/QMediaSync"
    } else {
        Write-Colored "red" "Error: arm64 executable not found"
    }
    
    if (Test-Path -Path "backend/web_statics" -PathType Container) {
        # 从assets目录复制db_config.html到web_statics目录
        if (Test-Path -Path "backend/assets/db_config.html" -PathType Leaf) {
            Copy-Item -Path "backend/assets/db_config.html" -Destination "backend/web_statics/" -Force
            Write-Colored "green" "✓ Copied db_config.html from assets to web_statics"
        } else {
            Write-Colored "yellow" "Warning: backend/assets/db_config.html not found"
        }
        Copy-Item -Path "backend/web_statics" -Destination "$arm64AppPath/" -Recurse -Force
        Write-Colored "green" "✓ Copied web_statics directory to backend/FNOS/qmediasync-arm64/app/"
    } else {
        Write-Colored "yellow" "Warning: web_statics directory not found"
    }
    
    # 切换到qmediasync-arm64目录并执行fnpack build
    Set-Location -Path "backend/FNOS/qmediasync-arm64"
...
if (!(Test-Path -Path "backend/FNOS" -PathType Container)) {
    ...
    # 创建app目录
    $amd64AppPath = "backend/FNOS/qmediasync-amd64/app"
    New-Item -Path $amd64AppPath -ItemType Directory -Force | Out-Null
    
    # 更新manifest文件中的version字段
    $amd64ManifestPath = "backend/FNOS/qmediasync-amd64/manifest"
    if (Test-Path -Path $amd64ManifestPath -PathType Leaf) {
        ...
        Write-Colored "green" "✓ Updated version in backend/FNOS/qmediasync-amd64/manifest to $TAG"
    } else {
    ...
    if (Test-Path -Path "temp_build/QMediaSync_linux_amd64_exe" -PathType Leaf) {
        ...
        Write-Colored "green" "✓ Copied amd64 executable to backend/FNOS/qmediasync-amd64/app/QMediaSync"
    } else {
    ...
    if (Test-Path -Path "backend/web_statics" -PathType Container) {
        ...
        Copy-Item -Path "backend/web_statics" -Destination "$amd64AppPath/" -Recurse -Force
        Write-Colored "green" "✓ Copied web_statics directory to backend/FNOS/qmediasync-amd64/app/"
    } else {
        Write-Colored "yellow" "Warning: web_statics directory not found"
    }
    
    # 切换到qmediasync-amd64目录并执行fnpack build
    Set-Location -Path "backend/FNOS/qmediasync-amd64"
    $fnpackExists = Get-Command "fnpack" -ErrorAction SilentlyContinue
    if ($fnpackExists) {
