...
func TestStrmConfigSource(t *testing.T) {
	tests := []struct {
		name         string
		customConfig bool
		want         string
	}{
		{name: "未开启自定义配置时使用全局设置", customConfig: false, want: "全局 STRM 设置"},
		{name: "开启自定义配置时使用同步目录设置", customConfig: true, want: "同步目录自定义设置"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := strmConfigSource(tt.customConfig); got != tt.want {
				t.Fatalf("strmConfigSource(%v) = %q，期望 %q", tt.customConfig, got, tt.want)
			}
		})
	}
}
