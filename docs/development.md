...
- 网盘文件浏览器使用 `/api/path/files` 拉取当前页，响应 `data` 为 `{ list, total, page, page_size }`；可获取目录总数的网盘应把服务端总数写入 `total`，前端分页页码不能用当前页 `list.length` 推断。页码或每页数量变化属于同目录翻页，加载期间保留上一份 `total`；只有账号或目录上下文切换才清空 `total`。
