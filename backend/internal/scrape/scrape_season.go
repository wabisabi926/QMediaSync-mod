...
func (t *tvShowScrapeImpl) UpdateSeasonDataToAllEpisodeBySeason(seasonMediaFile *models.ScrapeMediaFile) error {
	...
	if err != nil {
		helpers.AppLogger.Errorf("批量更新电视剧 %s 季 %d 的所有集新目录 ID 失败，失败原因：%v", seasonMediaFile.Name, seasonMediaFile.SeasonNumber, err)
		return err
	}
	helpers.AppLogger.Infof("批量更新电视剧 %s 季 %d 的所有集新目录 ID 成功", seasonMediaFile.Name, seasonMediaFile.SeasonNumber)
	return nil
}
...
func (t *tvShowScrapeImpl) MakeSeasonPath(seasonMediaFile *models.ScrapeMediaFile) error {
	...
	helpers.AppLogger.Infof("电视剧 %s 第 %d 季的目标目录 %s 创建成功", seasonMediaFile.Name, seasonMediaFile.SeasonNumber, destFullPath)
	return nil
}
...
func (t *tvShowScrapeImpl) ProcessSeason(seasonMediaFile *models.ScrapeMediaFile) error {
	...
	// 如果已经关联了 MediaSeason，且 MediaSeason 已刮削完毕，则跳过
...
func (t *tvShowScrapeImpl) ScrapeSeasonMedia(mediaFile *models.ScrapeMediaFile) error {
	...
	if mediaFile.ScrapeType != models.ScrapeTypeOnlyRename {
		...
		if mkdirErr := os.MkdirAll(localTempSeasonPath, 0777); mkdirErr != nil {
			...
			helpers.AppLogger.Infof("季临时刮削文件存储路径创建成功，电视剧 %s 第 %d 季：%s", mediaFile.Name, mediaFile.SeasonNumber, localTempSeasonPath)
		}
...
func (sm *tvShowScrapeImpl) GenerateSeasonNfo(mediaFile *models.ScrapeMediaFile) error {
	...
	if err != nil {
		helpers.AppLogger.Errorf("生成电视剧 %s 季 %d 的 NFO 文件失败：%v", mediaFile.Media.Name, mediaFile.MediaSeason.SeasonNumber, err)
		return err
	}
	helpers.AppLogger.Infof("生成电视剧 %s 季 %d 的 NFO 文件成功：%s", mediaFile.Media.Name, mediaFile.MediaSeason.SeasonNumber, seasonNfoFile)
	return nil
}
...
func (t *tvShowScrapeImpl) UpdateSeasonPathAndIdToAllEpisode(mediaFile *models.ScrapeMediaFile) error {
	...
	if affectedRows == 0 {
		helpers.AppLogger.Errorf("批量更新电视剧 %s 季 %d 的所有集信息失败，未更新任何行", mediaFile.Name, mediaFile.SeasonNumber)
