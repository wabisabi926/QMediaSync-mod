...
管理员用户名和密码在创建和修改时使用严格校验规则：用户名去除首尾空白后长度必须为 3 到 20 个字符且只能包含英文和数字，密码长度至少 6 个字符，且不能是纯数字或纯字母；修改密码时，新密码不能与当前密码相同。登录时校验用户名和密码非空，用户名 20 个字符上限。管理员密码使用 bcrypt 哈希保存，新生成和修改后的密码使用成本参数 `12`。旧成本哈希会在用户下一次成功登录后自动升级。
...
- 服务端通过 `user_sessions` 表控制会话有效性，退出登录、修改用户名或密码、两步验证变更和登录设备撤销都会更新该表。修改用户名或密码时，会在同一数据库事务中撤销全部浏览器 Cookie 会话（包括发起修改的当前浏览器）；成功响应会清除认证 Cookie，用户必须重新登录，重新登录后只会创建一个新的活动会话。单进程部署中，登录会话创建与凭据修改会串行处理，避免旧凭据在修改完成后创建新的活动会话；多个应用实例共享数据库时，需要改用跨实例锁或凭据版本机制。已撤销记录保留用于审计，不会显示在「登录设备」列表；当前设备始终排在列表首位，其他设备按最后活跃时间倒序。用户主动退出会请求 `POST /api/logout` 撤销服务端会话；已登录业务请求收到 `401` 时，前端只清理本地状态、关闭实时连接并跳转登录页，不会再请求 `/api/logout`。CSRF 校验失败仍返回 `403`。API Key 不受凭据修改的浏览器会话撤销规则影响。
