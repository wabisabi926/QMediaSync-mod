...
请求必须提供 `sync_path_id`。单文件请求至少提供 `file_id` 或 `path + file_name` 中的一组远端定位信息，`pick_code` 可以作为辅助字段传入但不能单独定位文件；批量请求使用 `items`；目录请求使用 `directory_id` 或 `directory_path`。`path` / `directory_path` 只表示远端路径，禁止通过 `local_path` 指定本地写入位置。
