...
- 目录监控规则启动后只负责 fsnotify / polling 发现文件和稳定性检查；符合条件的文件会创建 `directory_monitor` 上传任务，真实上传仍由全局上传队列处理。默认只处理当前同步目录视频扩展名文件；`upload_metadata=true` 时同时处理当前同步目录元数据扩展名文件。视频和元数据扩展名都使用同步目录自定义配置优先、为空回退全局 STRM 设置的规则。
