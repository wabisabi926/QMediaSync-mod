...
func (r *RenameLocal) move(mediaFile *models.ScrapeMediaFile, destPathId, newName string) error {
	...
	if mediaFile.ScrapeType == models.ScrapeTypeOnlyRename && mediaFile.MediaType == models.MediaTypeOther {
		...
		if mediaFile.NfoFileId != "" {
			...
			if err != nil {
				helpers.AppLogger.Errorf("移动 NFO 文件 %s 到 %s 失败：%v", mediaFile.NfoFileName, destPathId+"/"+newNfoName, err)
			} else {
				helpers.AppLogger.Infof("NFO 文件 %s 成功移动到 %s", mediaFile.NfoFileId, destPathId+"/"+newNfoName)
			}
		}
	}
	return nil
...
func (r *RenameLocal) copy(mediaFile *models.ScrapeMediaFile, destPathId, newName string) error {
	...
	destFullPath := filepath.Join(destPathId, newName)
	if helpers.PathExists(destFullPath) {
		helpers.AppLogger.Infof("文件 %s 已存在，无需复制", destFullPath)
	} else {
		err := helpers.CopyFile(sourceFullPath, destFullPath)
		if err != nil {
			helpers.AppLogger.Errorf("复制文件失败：%v", err)
			return err
		} else {
			helpers.AppLogger.Infof("文件 %s 成功复制到 %s", sourcePath+"/"+mediaFile.VideoFilename, destPathId+"/"+newName)
		}
	}
	if mediaFile.MediaType != models.MediaTypeTvShow {
		mediaFile.Media.VideoFileId = destFullPath
	...
	if mediaFile.SubtitleFileJson != "" {
		...
		for _, sub := range mediaFile.SubtitleFiles {
			...
			if err != nil {
				helpers.AppLogger.Errorf("复制字幕文件 %s 到 %s 失败：%v", sub.FileName, destPathId+"/"+newSubName, err)
			} else {
				helpers.AppLogger.Infof("字幕文件 %s 成功复制到 %s", sub.FileId, destPathId+"/"+newSubName)
				newSub := &models.MediaMetaFiles{
					FileName: newSubName,
					FileId:   newSubFullPath,
					PickCode: newSubFullPath,
	...
	if mediaFile.ScrapeType == models.ScrapeTypeOnlyRename && mediaFile.MediaType == models.MediaTypeOther {
		...
		if mediaFile.ImageFilesJson != "" {
			for _, imageFile := range mediaFile.ImageFiles {
				...
				if err != nil {
					helpers.AppLogger.Errorf("复制图片文件 %s 到 %s 失败：%v", imageFile.FileName, destPathId+"/"+newImageName, err)
				} else {
					helpers.AppLogger.Infof("图片文件 %s 成功复制到 %s", imageFile.FileId, destPathId+"/"+newImageName)
				}
			}
		}
		// 复制 NFO 文件
		if mediaFile.NfoFileId != "" {
			...
			if err != nil {
				helpers.AppLogger.Errorf("复制 NFO 文件 %s 到 %s 失败：%v", mediaFile.NfoFileName, destPathId+"/"+newNfoName, err)
			} else {
				helpers.AppLogger.Infof("NFO 文件 %s 成功复制到 %s", mediaFile.NfoFileId, destPathId+"/"+newNfoName)
			}
		}
	}
	return nil
...
func (r *RenameLocal) symlink(mediaFile *models.ScrapeMediaFile, destPathId, newName string, isHard bool) error {
	...
	destFullPath := filepath.Join(destPathId, newName)
	if helpers.PathExists(destFullPath) {
		helpers.AppLogger.Infof("文件 %s 已存在，无需创建链接", destFullPath)
	} else {
		var err error
		if isHard {
		...
		if err != nil {
			helpers.AppLogger.Errorf("创建链接失败：%v", err)
			return err
		} else {
	...
	if mediaFile.SubtitleFileJson != "" {
		...
		for _, sub := range mediaFile.SubtitleFiles {
			...
			if err != nil {
				helpers.AppLogger.Errorf("创建字幕文件链接 %s 到 %s 失败：%v", sub.FileName, destPathId+"/"+newSubName, err)
			} else {
	...
	if mediaFile.ScrapeType == models.ScrapeTypeOnlyRename && mediaFile.MediaType == models.MediaTypeOther {
		...
		if mediaFile.ImageFilesJson != "" {
			for _, imageFile := range mediaFile.ImageFiles {
				...
				if err != nil {
					helpers.AppLogger.Errorf("创建图片文件链接 %s 到 %s 失败：%v", imageFile.FileName, destPathId+"/"+newImageName, err)
				} else {
					helpers.AppLogger.Infof("图片文件 %s 成功链接到 %s", imageFile.FileId, destPathId+"/"+newImageName)
				}
			}
		}
		// 创建 NFO 文件链接
		if mediaFile.NfoFileId != "" {
			...
			if err != nil {
				helpers.AppLogger.Errorf("创建 NFO 文件链接 %s 到 %s 失败：%v", mediaFile.NfoFileName, destPathId+"/"+newNfoName, err)
			} else {
				helpers.AppLogger.Infof("NFO 文件 %s 成功链接到 %s", mediaFile.NfoFileId, destPathId+"/"+newNfoName)
			}
		}
	}
	return nil
...
func (r *RenameLocal) RemoveMediaSourcePath(mediaFile *models.ScrapeMediaFile, sp *models.ScrapePath) error {
	...
	if len(dirEntries) == 0 || sp.ForceDeleteSourcePath {
		...
		helpers.AppLogger.Infof("刮削完成，已尝试删除本地目录，路径：%s", sourcePath)
	}
	// 如果有电视剧文件夹，则删除
	if mediaFile.PathId != "" {
		...
		if len(dirEntries) == 0 || sp.ForceDeleteSourcePath {
			...
			helpers.AppLogger.Infof("刮削完成，已尝试删除本地电视剧文件夹，路径：%s", mediaFile.TvshowPathId)
		}
	}
	return nil
}
...
func (r *RenameLocal) ExistsAndRename(fileId, newName string) (string, error) {
	...
	if filepath.Base(fileId) == newName {
		helpers.AppLogger.Infof("本地文件名未变化，无需重命名：路径：%s", fileId)
		return fileId, nil
	}
	// 重命名文件
