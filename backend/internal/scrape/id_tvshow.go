...
func (i *IdTvShowImpl) Identify(mediaFile *models.ScrapeMediaFile) error {
	// 如果已有 TMDB ID，则不继续识别；重新识别会自动填入 TMDB ID。
	...
	if err != nil {
		reason := err.Error()
		if err.Error() == "多条记录" {
			reason = "通过名称和年份查到多部电视剧，请手工重新识别并指定准确的 TMDB ID"
		}
...
func (i *IdTvShowImpl) extractInfo(mediaFile *models.ScrapeMediaFile) (*helpers.MediaInfo, error) {
	...
	if i.scrapePath.EnableAi == models.AiActionEnforce {
		// 强制使用 AI，如果 AI 失败则退回正则识别
		disableAI = true
		aiInfo, err := i.extractInfoByAI(mediaFile)
		if err != nil {
			helpers.AppLogger.Errorf("使用 AI 提取电视剧信息失败，退回正则识别：%v", err)
		} else if aiInfo != nil {
			helpers.AppLogger.Infof("强制使用 AI 提取电视剧信息成功，文件名：%s，文件夹：%s，提取结果：%+v", mediaFile.VideoFilename, filepath.Base(mediaFile.TvshowPath), aiInfo)
	...
	if err != nil {
		...
		if disableAI || i.scrapePath.EnableAi == models.AiActionOff {
			helpers.AppLogger.Errorf("由于禁用 AI 识别，所以直接返回错误：%v", err)
			return nil, err
		}
	...
	if assistErr != nil {
		helpers.AppLogger.Errorf("使用 AI 辅助提取电视剧信息失败，退回正则识别：%v", assistErr)
	...
	return nil, errors.New("使用 AI 辅助从文件名中提取媒体名称和年份失败，未找到匹配记录")
}
...
func (i *IdTvShowImpl) extractInfoByAI(mediaFile *models.ScrapeMediaFile) (*helpers.MediaInfo, error) {
	...
	if err != nil {
		helpers.AppLogger.Errorf("强制使用 AI 从文件名中提取媒体信息失败：%v", err)
		return nil, err
	}
	if info.Name != "" && info.Year != 0 {
		...
		if cerr != nil {
			helpers.AppLogger.Errorf("AI 从文件名中查询名称和年份失败，文件名：%s，提取结果：%+v，错误信息：%v", mediaFile.VideoFilename, info, cerr)
		}
		if checkId > 0 {
			helpers.AppLogger.Infof("AI 从文件名中查询名称和年份成功，文件名：%s，提取结果：%+v，名称：%s，TMDB ID：%d", mediaFile.VideoFilename, info, name, checkId)
			// 查到了，直接用
			return &helpers.MediaInfo{
				Name:   name,
				Year:   cyear,
	...
	// 从文件夹中提取信息
	helpers.AppLogger.Warnf("AI 从文件名中提取的媒体信息不完整，继续从文件夹中补齐，文件名：%s，提取结果：%+v", mediaFile.VideoFilename, info)
	folderInfo, err := client.TakeMoiveName(folderName, i.scrapePath.GetAiPrompt())
	if err != nil {
		helpers.AppLogger.Errorf("AI 从文件夹中提取媒体信息失败，文件夹：%s，错误信息：%v", folderName, err)
		return nil, err
	}
	helpers.AppLogger.Infof("AI 从文件夹中提取媒体信息，文件夹：%s，提取结果：%+v", folderName, folderInfo)
	...
	if err != nil {
		helpers.AppLogger.Errorf("AI 从文件夹中提取媒体信息后查询名称和年份失败，文件夹：%s，提取结果：%+v，错误信息：%v", folderName, folderInfo, err)
		return nil, err
	}
	if checkId > 0 {
		helpers.AppLogger.Infof("AI 从文件夹中提取媒体信息后查询名称和年份成功，文件夹：%s，提取结果：%+v，名称：%s，TMDB ID：%d", folderName, folderInfo, name, checkId)
		// 查到了，直接用
		return &helpers.MediaInfo{
			Name:   name,
			Year:   cyear,
	...
	return nil, errors.New("AI 从文件夹中提取媒体信息后查询名称和年份失败，未找到匹配记录")
}
...
func (i *IdTvShowImpl) extractInfoByRE(mediaFile *models.ScrapeMediaFile) (*helpers.MediaInfo, error) {
	...
	// 	if cerr != nil {
	// 		helpers.AppLogger.Errorf("使用 TMDB ID 查询媒体信息失败，TMDB ID：%d，错误信息：%v", info.TmdbId, cerr)
	// 	} else {
	// 		if cname != "" {
	// 			info.Name = cname
	// 		}
	...
	// 		info.Name = cname
	// 		info.Year = cyear
	// 		helpers.AppLogger.Infof("使用 TMDB ID 查询媒体信息成功，TMDB ID：%d，名称：%s，年份：%d", info.TmdbId, info.Name, info.Year)
	// 		return info, nil
	// 	}
	// }
	// if info.Name != "" {
	...
	// 		info.Year = cyear
	// 		info.Name = cname
	// 		helpers.AppLogger.Infof("使用名称和年份查询媒体信息成功，名称：%s，年份：%d，TMDB ID：%d", info.Name, info.Year, info.TmdbId)
	// 		return info, nil
	// 	}
	// }
...
// 先从文件名中提取名称和年份；如果有结果则直接使用；否则用名称搜索，再结合季信息校验年份。
// 如果上述流程无法完成，则从文件夹中提取名称和年份。
...
func (i *IdTvShowImpl) find(mediaFile *models.ScrapeMediaFile, tmdbid int64, name string, year int) (*helpers.MediaInfo, error) {
	if tmdbid != 0 {
		...
		if cerr != nil {
			helpers.AppLogger.Errorf("使用 TMDB ID 查询媒体信息失败，TMDB ID：%d，错误信息：%v", tmdbid, cerr)
		} else {
			if cname != "" {
				name = cname
			}
	...
	if name != "" {
		...
		if year != 0 && mediaFile.SeasonNumber >= 0 {
			...
			if cerr != nil {
				helpers.AppLogger.Errorf("使用 TMDB ID 查询季失败，TMDB ID：%d，季数：%d，错误信息：%v", cid, mediaFile.SeasonNumber, cerr)
			} else {
				if season.AirDate != "" {
					...
					if syear != 0 {
						// 检查年份是否相同
						if syear != year {
							helpers.AppLogger.Errorf("TMDB ID %d，季数 %d，季年份 %d 与文件年份 %d 不匹配", cid, mediaFile.SeasonNumber, syear, year)
							return nil, fmt.Errorf("TMDB ID %d，季数 %d，季年份 %d 与文件年份 %d 不匹配", cid, mediaFile.SeasonNumber, syear, year)
						}
					}
				}
				return &helpers.MediaInfo{
	...
	return nil, fmt.Errorf("名称 %s，年份 %d，无法从 TMDB 查询到匹配的电视剧信息", name, year)
}
