...
- 目录监控规则启动后只负责 watcher / polling 发现文件和稳定性检查；符合条件的文件会创建 `directory_monitor` 上传任务，真实上传仍由全局上传队列处理。
- `watch_mode=auto` 会在 watcher 初始化失败时回退 polling；`watch_mode=watcher` 初始化失败则规则启动失败。
