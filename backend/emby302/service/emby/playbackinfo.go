...
func LoadCacheItems(c *gin.Context) {
	...
	coverMediaSources := func(info *jsons.Item) bool {
		cacheMs, iok := info.Attr("MediaSources").Done()
		if !iok || cacheMs.Type() != jsons.JsonTypeArr {
			return false
		}
	...
	spaceCache, ok := getPlaybackInfoByCacheSpace(itemInfo)
	if ok {
		cacheBody, serr := spaceCache.JsonBody()
		if serr == nil && coverMediaSources(cacheBody) {
			return
		}
	}
...
func fetchFullPlaybackInfo(itemInfo ItemInfo) (*jsons.Item, error) {
	...
	// logs.Success("获取全量 PlaybackInfo 成功, PlaybackInfo: %+v", bodyJson)
	return bodyJson, nil
}
